...
const build_droppable = (name, data_points, load_callback) => {
  ...
  return <Droppable isDropDisabled={['left', 'center', 'right'].includes(name)} droppableId={name} direction="vertical">
  {(provided, snapshot) => (
    <div
          ...
          {(provided, snapshot) => (
            ...
            >
              <OnImagesLoaded
                ...
                onTimeout={() => {console.log("unable to load image: /api/image/"+item.model+"/"+item.version)}}
                timeout={7000}
              >
                <img id={name} src={"/api/image/"+item.filename} alt={item.filename} style={{height: '20vw',  width: '20vw', minWidth: '100px',  minHeight: '100px'}}/>
              </OnImagesLoaded>              
            </div>
          )}
        </Draggable>
...
function SampleTask() {
  ...
  const submitData=()=>{
    let best = items[3];
    let worst = items[4];
    let second = items[0].length ? items[0] : (items[1].length ? items[1] : items[2])
    best = best[0]
    worst = worst[0]
    second = second[0]
  ...
  return (<>
          <div style={{height: "100%", position: 'absolute', overflow: 'hidden', backgroundColor: "white", width: n_loaded_images>=2?'0%':'100%', opacity: 0.7}} ></div>
          <div class="center" style={{width: "100%"}}>Remaining tasks: {data["batch"].length}</div>
          <DragDropContext onDragEnd={onDragEnd}>
            
            <div id="content-container" style={{margin: 'auto', padding: '10px', width: "fit-content"}}><table class="tg">
            <thead>
            <tr>
            ...
            <tr>
                ...
                <td>
                  <div style={{margin:'auto', display: has_selected_best_and_worst() ? true : "None"}}>
                    <div style={{margin: '0 auto', width:'100px'}}>
                    <Button id="confirm_button" variant="contained" color="primary" onClick={submitData} style={{width:'100%'}}>Submit</Button>
                    </div>
                  </div>
                </td>
